...
function td_gallery_shortcode($output = '', $atts, $content = false) {
    ...
    if(!empty($atts) and !empty($atts['td_select_gallery_slide']) and $atts['td_select_gallery_slide'] == 'slide') {
        ...
        if(!empty($slide_display_html) and !empty($slide_cursor_html)) {
            wp_enqueue_script('banners-galerias', td_global::$get_template_directory_uri . '/js/galerias.js', array(), TD_THEME_VERSION, false);
                ...
                <div id="' . $gallery_slider_unique_id . '" class="' . $td_nr_columns_slide . '">
                    <div class="post_td_gallery">
                        ...
                        <div class = "td-doubleSlider-1">
                            <div class="advertise" style="display:none;text-align:center;width:100%">
    							<div id="ban02_300x250" class="banner">	</div>
    						    <a class="skip" href="#" title="Omitir anuncio">
    								<p>Omitir anuncio <span><i class="tvsagal-doble-derecha"></i></span></p>
    							</a>
    						</div>
            ...
            $slide_javascript = '
                    ...
                    jQuery(document).ready(function() {
                        ...
                        function doubleSlider2Load_' . $gallery_slider_unique_id . '(args) {
                            ...
                            count_banner++;
                            if(currentSlide!=' . $gallery_slider_unique_id . '_nr_of_slides && boxBannerPlayer){
                               	console.log(count_banner);
                               if (count_banner == 5) {
                                 advertise = 1;
                                // jQuery(".td-doubleSlider-1").hide();
                                 jQuery(".advertise").fadeIn(1000);
                                 time_pub = setTimeout(function() {
                                       jQuery(".advertise").hide();
                                       //jQuery(".td-doubleSlider-1").show();
                                       advertise = 0;
                                       interBanner();
                                 }, 8000);
                                 count_banner = 0;
